-- Repair deployments still running the original JSONB/SQL-array assignment
-- function. Keep assignment, review routing, audit and permissions unchanged.
-- Expand assign_task's returned row before assigning it to the tasks record;
-- this supports both composite and SETOF tasks deployments.
...
begin
  ...
  if before_task.assigned_to is distinct from p_assignee
    ...
    select * into assigned_task from public.assign_task(
      p_task_id,
      p_assignee,
      p_assignee_name
    );
