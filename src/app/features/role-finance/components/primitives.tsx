...
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  return (
    <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <Text className="mb-1 flex items-center gap-2 uppercase tracking-[0.08em] text-neutral-500" type="text3" weight="medium"><CreditCard aria-hidden="true" size={14} /> Finance · Operational Ledger</Text>
        <Heading className="text-neutral-900" type="h1" weight="medium">{title}</Heading>
        <Text className="mt-1 text-neutral-500" type="text2">{subtitle || "Office of the City Accountant & Treasurer · Ormoc City"}</Text>
      </div>
...
export function Btn({ icon, label, variant = "secondary", onClick, disabled }: { icon?: ReactNode; label: string; variant?: "primary" | "secondary" | "danger" | "success"; onClick?: () => void; disabled?: boolean }) {
  ...
  return <Button color={styles[variant].color} disabled={disabled} kind={styles[variant].kind} onClick={() => onClick?.()} size="small">{icon && <span aria-hidden="true" className="inline-flex">{icon}</span>}{label}</Button>;
}
...
export function Stat({ label, value, trend, tone = "neutral" }: { label: string; value: string; trend?: string; tone?: "neutral" | "good" | "warn" | "bad" }) {
  ...
  return <section aria-label={label} className="rounded-xl border border-neutral-200 bg-white p-4"><Text className="uppercase tracking-[0.08em] text-neutral-500" type="text3" weight="medium">{label}</Text><div className={`eflow-tabular mt-1 text-[22px] font-semibold ${tones[tone]}`}>{value}</div>{trend && <Text className="mt-1 text-neutral-500" type="text3">{trend}</Text>}</section>;
}
