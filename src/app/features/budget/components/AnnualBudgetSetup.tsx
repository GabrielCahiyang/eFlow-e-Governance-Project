...
export function AnnualBudgetSetup({ orgId, fiscalYear, data, canEdit, onChanged }: { orgId: string; fiscalYear: number; data: DepartmentBudgetBundle; canEdit: boolean; onChanged: () => Promise<void> }) {
  ...
  return <div className="space-y-4">
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-[9px] uppercase tracking-[0.18em] text-neutral-400">Fiscal year {fiscalYear}</div><h2 className="mt-1 text-[15px] font-semibold text-neutral-900">Annual department budget</h2><p className="mt-1 text-[10.5px] text-neutral-500">The Department Head defines the annual envelope once. Published proposals reserve funds from this locked total.</p></div><div className="text-right"><div className="text-[9px] uppercase tracking-wide text-neutral-400">Annual total</div><div className="mt-1 text-[20px] font-semibold text-neutral-950">{peso.format(total)}</div></div></div>
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
      ...
      <label className="mt-4 flex items-start gap-3 rounded-xl border border-neutral-200 bg-neutral-50 p-3"><input type="checkbox" disabled={!canEdit || locked} checked={allowReceiptOverride} onChange={(event) => setAllowReceiptOverride(event.target.checked)} className="mt-0.5" /><ReceiptText size={14} className="mt-0.5 text-neutral-500" /><span><span className="block text-[10.5px] font-medium text-neutral-800">Allow justified receipts above the review threshold</span><span className="mt-0.5 block text-[9.5px] text-neutral-500">The threshold is not a request cap. When enabled, a receipt above it requires a written exception reason and remains visible to both reviewers.</span></span></label>
    ...
    {canEdit && !locked && <div className="flex justify-end gap-2"><button disabled={busy} onClick={save} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-neutral-200 bg-white px-4 text-[10.5px] font-medium text-neutral-700"><Save size={12} /> Save draft</button><button disabled={busy || total <= 0} onClick={lock} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-neutral-950 px-4 text-[10.5px] font-medium text-white disabled:opacity-40"><LockKeyhole size={12} /> Save &amp; lock annual budget</button></div>}
