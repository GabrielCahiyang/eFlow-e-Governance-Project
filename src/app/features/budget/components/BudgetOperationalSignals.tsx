...
export function BudgetOperationalSignals({ data }: { data: DepartmentBudgetBundle }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-3">
    <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm xl:col-span-2">
      <header className="mb-4 flex items-center gap-2"><TrendingUp size={14} /><div><h3 className="text-[11px] font-semibold">Proposal commitment versus actual</h3><p className="mt-0.5 text-[9px] text-neutral-400">Verified spending only; draft and unliquidated cash are excluded.</p></div></header>
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
      <header className="mb-3 flex items-center gap-2"><Banknote size={14} /><h3 className="text-[11px] font-semibold">Upcoming cash releases</h3></header>
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
      <header className="mb-3 flex items-center gap-2"><Clock3 size={14} /><h3 className="text-[11px] font-semibold">Oldest outstanding cash</h3></header>
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm xl:col-span-2">
      <header className="mb-3 flex items-center gap-2"><History size={14} /><h3 className="text-[11px] font-semibold">Recent financial activity</h3></header>
      {data.ledger.length ? <div className="grid gap-2 sm:grid-cols-2">{data.ledger.slice(0, 6).map((entry) => <div key={entry.id} className="flex gap-2 rounded-xl border border-neutral-100 bg-neutral-50 p-3"><div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white text-neutral-500"><History size={10} /></div><div className="min-w-0"><div className="truncate text-[9.5px] font-medium capitalize">{entry.entryType.replace(/_/g, " ")}</div><div className="mt-0.5 truncate text-[8.5px] text-neutral-400">{entry.description}</div><div className="mt-1 text-[8.5px] text-neutral-500">{peso.format(entry.amount)} · {new Date(entry.createdAt).toLocaleString()}</div></div></div>)}</div> : <EmptyLine text="Audited financial actions will appear here." />}
    </section>
  </div>;
}
