...
function HealthMetric({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number; tone: "neutral" | "red" | "amber" | "green" }) {
  ...
  return <div className={`rounded-xl p-3 ${styles}`}><div className="flex items-center justify-between"><span className="text-[9px] uppercase tracking-wide opacity-70">{label}</span>{icon}</div><div className="mt-1 text-[21px] font-semibold">{value}</div></div>;
}

function Quality({ label, value, hint }: { label: string; value: string; hint: string }) {
  return <div className="rounded-lg border border-neutral-100 bg-neutral-50/60 p-3"><div className="flex items-center gap-1 text-[9px] uppercase tracking-wide text-neutral-400"><TimerReset size={11} />{label}</div><div className="mt-1 text-[16px] font-semibold text-neutral-900">{value}</div><div className="mt-0.5 text-[9px] text-neutral-400">{hint}</div></div>;
}
