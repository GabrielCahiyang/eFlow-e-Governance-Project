...
export function GovernanceTimelinePanel({ revisions, approvals, changes, signoffs, records, closeout, closeoutDecisions, tasks, organizations, profiles }: {
  ...
  return <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><div className="text-[16px] font-semibold text-neutral-900">Unified governance timeline</div><p className="mt-1 text-[12px] text-neutral-500">Revision history, organization decisions, named sign-offs, changes, task evidence submissions, formal records, and closeout in one chronology.</p><div className="mt-4 space-y-1">{items.length ? items.map((item) => <div key={item.id} className="grid grid-cols-[28px_minmax(0,1fr)_130px] items-start gap-2 rounded-xl px-2 py-2 hover:bg-neutral-50"><span className={`flex h-7 w-7 items-center justify-center rounded-full ${item.tone === "good" ? "bg-emerald-50 text-emerald-700" : item.tone === "warn" ? "bg-amber-50 text-amber-700" : "bg-neutral-100 text-neutral-500"}`}>{item.icon}</span><div><div className="text-[13px] font-medium capitalize text-neutral-800">{item.title}</div><div className="mt-0.5 text-[12px] leading-relaxed text-neutral-500">{item.detail}</div></div><time className="pt-1 text-right text-[12px] text-neutral-500">{new Date(item.at).toLocaleString()}</time></div>) : <div className="rounded-xl border border-dashed border-neutral-300 p-5 text-center text-[12px] text-neutral-500">Governance activity will appear here.</div>}</div></section>;
}
